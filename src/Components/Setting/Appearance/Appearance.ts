import { changeTransparentEffect, enableShadowEffect, getAvailableFonts } from "../../../Service/app";
...
import { close, maximize, minimize, reload } from "../../Layout/windowManager";
...
const Appearance = async (): Promise<void> => {
    ...
    const fontSize = Number.parseInt(_appearance?.fontSize ?? 16);
    const windowTransparency = Number.parseInt(_appearance?.windowTransparency ?? 80);
	...
	<select name="transparent-effect">
		<option value="blur" ${transparentEffect === "blur" ? "selected" : ""} ${
            platform !== "win32" ? "disabled" : ""
        } ${disabledForWeb}>${blur_i18n}</option>
    ...
    settingsMain.querySelector('[name="fontSize"]')?.addEventListener("input", (event: Event & { target: HTMLInputElement }) => {
        ...
        document.documentElement.style.fontSize = `${Number.parseInt(event.target.value)}px`;
        Storage.set("appearance", appearance);
    });
