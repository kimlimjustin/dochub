...
const New = async (type: "file" | "folder"): Promise<void> => {
    ...
    if (type === "file") {
        Ask("New File", "File Name:").then(async (fileName: string) => NewFile(fileName, await focusingPath()));
    } else if (type === "folder") {
        Ask("New Folder", "Folder Name:").then(async (folderName: string) => NewFolder(folderName, await focusingPath()));
    }
};
